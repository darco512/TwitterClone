...
class GroupController extends Controller
{
    ...
    public function approveInvitation(string $token)
    {
        ...
        if(!$groupUser) {
            $errorTitle = 'The token is not valid';
        } else if ($groupUser->token_used) {
            $errorTitle = 'The token is not used';
        } else if ($groupUser->token_expire_date <Carbon::now()) {
            $errorTitle = 'The token is expired';
    ...
    public function join(Group $group)
    {
        ...
        if(!$group->auto_approval) {
            $status =  GroupUserStatus::PENDING->value;
            $successMessage = 'You have requested to joined to group "'.$group->name.'"';
    ...
    public function changeRole(Request $request, Group $group)
    {
        ...
        if($group->isOwner($user_id)) {
            return response('You can not change role of the owner of the group', 403);
        }
    ...
    public function removeUser(Request $request, Group $group)
    {
        ...
        if($group->isOwner($user_id)) {
            return response('You can not remove the owner of the group', 403);
        }
